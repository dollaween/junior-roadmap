...
Оператор `!` — преобразовывает `true` в `false`, и наоборот. Если значение не является логическим — то сперва преобразует его в логическое значение, а затем `true` в `false` (или `false` в `true`).
